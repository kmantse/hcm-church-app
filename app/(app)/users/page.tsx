...
export default function UsersPage() {
  ...
  const openEdit = (u: UserRecord) => { setEditTarget(u); setForm({ name: u.name, email: u.email, password: "", role: u.role }); setError(""); };
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/users/${editTarget.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      const data = await res.json();
      if (!res.ok) { setError(data.error); return; }
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <FormField label={isEdit ? "New Password (leave blank to keep current)" : "Password"} required={!isEdit}>
        <div className="relative">
          <input type={showPassword ? "text" : "password"} value={form.password} onChange={set("password")} className={`${inputClass} pr-10`} placeholder={isEdit ? "Leave blank to keep current" : "Min 8 characters"} required={!isEdit} minLength={8} />
      ...
      <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-1">
        <button type="button" onClick={() => { setShowCreate(false); setEditTarget(null); }} className="px-4 py-2 border border-slate-300 rounded-lg text-sm hover:bg-slate-50">Cancel</button>
    ...
    <div className="p-4 sm:p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {ROLES.map((role) => (
          <div key={role} className="bg-white rounded-xl border border-slate-200 p-4">
            <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full mb-2 ${ROLE_COLORS[role]}`}>{ROLE_LABELS[role]}</span>
            <p className="text-xs text-slate-500">{ROLE_DESCRIPTIONS[role]}</p>
            <p className="text-xs font-semibold text-slate-700 mt-1">{users.filter((u) => u.role === role && u.isActive).length} active</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        {users.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {users.map((u) => (
              <div key={u.id} className="flex items-center gap-3 px-4 sm:px-5 py-4 hover:bg-slate-50 transition-colors">
                {/* Avatar */}
                <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${u.isActive ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-400"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <p className={`text-sm font-semibold truncate ${u.isActive ? "text-slate-900" : "text-slate-400"}`}>{u.name}</p>
                    {currentUser?.id === u.id && <span className="text-xs bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-full shrink-0">You</span>}
                    {!u.isActive && <span className="text-xs bg-red-50 text-red-500 px-1.5 py-0.5 rounded-full shrink-0">Inactive</span>}
                  </div>
                  <p className="text-xs text-slate-500 truncate">{u.email}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${ROLE_COLORS[u.role as keyof typeof ROLE_COLORS] ?? "bg-slate-100 text-slate-600"}`}>
                      {ROLE_LABELS[u.role as keyof typeof ROLE_LABELS] ?? u.role}
                    </span>
                    <span className="text-xs text-slate-400 hidden sm:inline">Added {formatDate(u.createdAt)}</span>
                  </div>
                </div>

                {/* Actions */}
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => openEdit(u)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit user">
                    <Edit2 className="w-3.5 h-3.5" />
                  </button>
                  {currentUser?.id !== u.id && (
                    <>
                      <button onClick={() => handleToggleActive(u)} className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title={u.isActive ? "Deactivate" : "Activate"}>
                        {u.isActive ? <ToggleRight className="w-4 h-4 text-green-500" /> : <ToggleLeft className="w-4 h-4" />}
                      </button>
                      <button onClick={() => setDeleteTarget(u)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete user">
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete User" size="sm">
        <div className="text-center py-2">
          ...
          <p className="text-sm text-slate-500 mt-1">This will permanently remove their account.</p>
        </div>
